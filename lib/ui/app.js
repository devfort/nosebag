...
class App extends Component {
  ...
  render() {
    ...
    if (this.state.screen == 'search') {
      screen = <Search add_recipe={ (recipe) => this.add_recipe_to_list(recipe) } show_recipe={ (recipe) => this.show_recipe(recipe) } />;
    } else if (this.state.screen == 'recipe') {
      screen = <Recipe recipe={ this.state.recipe } />;
    } else if (this.state.screen == 'shopping') {
    ...
    return <>
      <header>
      <h1 onClick={ () => this.setState({'screen': 'about'}) }><button>Nosebag</button></h1>
      <nav>
        <ol>
