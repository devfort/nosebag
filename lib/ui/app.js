...
class App extends Component {
  ...
  update_shopping_items_count() {
    this.setState({ shopping_items: this.state.list.item_count() });
  }
  ...
  render() {
    ...
    if (this.state.screen == 'search') {
      ...
      screen = <ShoppingList inc_recipe={ (recipe) => this.add_recipe_to_list(recipe) } dec_recipe={ (recipe) => this.remove_recipe_from_list(recipe) } list={ this.state.list } />;
    }
    return <>
      <header>
      <h1>Nosebag</h1>
