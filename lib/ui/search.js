...
class SearchResultQuantity extends Component {
  render() {
...
class SearchResult extends Component {
  render() {
    return <>
      ...
      <h3>Ingredients</h3>
      <ol>
      { this.props.recipe.ingredients.map( (ri, idx) => <li key={ idx }><SearchResultQuantity quantity={ ri.quantity } /> { ri.ingredient.name }</li> ) }
      </ol>
      </section>
      </>;
  }
...
class Search extends Component {
  ...
  async runSearch(query_string) {
    ...
    console.log("searched", query_string, "found", results.map(x => x.id));
    this.setState(
      {
        results: results,
      }
  ...
  render() {
    ...
    if (this.state.query == '') {
      ...
      results_or_examples = <SearchResults setSearch={(x) => this.setSearch(x)} results={this.state.results} />;
    }
