const { QuantityTerm, Term, And, AndNot, Any } = require('./nodes');
...
class Parser {
  ...
  parse_quantity_term() {
    return this.capture('quantity', () => {
      let words = [];
      let quantity = this.read('quantity');
      if (!quantity) return null;

      let q_match = quantity.token.match(/^(\d+)(.*)$/);
      let amount = parseInt(q_match[1]);
      let unit = q_match[2] || null;
      ...
      return new QuantityTerm(
        amount,
        unit,
        words.map(w => w.token).join(' ')
      );
    })
  }
...
module.exports = {
  parse,
  QuantityTerm, Term, And, AndNot, Any
};
